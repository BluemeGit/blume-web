        ...
        `}>
            ...
            `}>
                <div css={css`
                    margin-bottom: 48px;
                `}>
                    <ToggleButtonPC selected={!isWish} onClick={() => setIsWish(false)}>전체 제품</ToggleButtonPC>
                    <ToggleButtonPC selected={isWish} onClick={() => setIsWish(true)}>찜한 제품</ToggleButtonPC>
                </div>
                <div css={css`
                    display: grid;
                ...
                `}>
                    {products && ads && products.data.map((product: ProductType, id: number) => {
                        ...
                        return (
                            <>
                                <div css={css`cursor: pointer;`} onClick={() => navigate(`/products/${product.id}`)}>
                                    <img 
                                        src={'https://kr.object.ncloudstorage.com/blume/' + product.imageUrl} 
                                        alt={'상품이미지'}
                                        css={css`
                                            width: 235px;
                                            height: 235px;
